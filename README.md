...
- [DoubletCollection](https://github.com/xnnba1984/DoubletCollection) - R package accompanying the benchmarking review for installing eight doublet detection methods and detecting doublets in 16 experimental and many simulated (scDesign, Splatter) data with doublet annotations. Protocol for downstream analyses and performance evaluation. scDblFinder has excellent detection accuracy and high computational efficiency. Preprocessed data as RDS objects on [Zenodo](https://doi.org/10.5281/zenodo.4562782). <details>
  <summary>Paper</summary>
  Xi, Nan Miles, and Jingyi Jessica Li. “Protocol for Executing and Benchmarking Eight Computational Doublet-Detection Methods in Single-Cell RNA Sequencing Data Analysis.” STAR Protocols 2, no. 3 (2021): 100699. https://doi.org/10.1016/j.xpro.2021.100699.
</details>

- [COMPOSITE](https://github.com/CHPGenetics/COMPOSITE) - COMpound POiSson multIplet deTEction, a compound Poisson model-based framework for multiplet detection in single-cell multiomics data. Considers doublets and triplets. Utilizes stable features since their values are more closely related to multiplet status compared to highly variable features (method from scMerge, 300 features for RNA and ATAC modalities, fop 10% for ADT). Trimodal DOGMA-seq (scRNA-seq, antibody-derived tags (ADT), scATAC-seq) experiments with cell hashing data as ground truth (17 datasets), simulated data. scATAC-seq represented by gene activity inferred by Signac. For scRNA-seq and scATAC-seq, the contribution of a single cell to each stable feature follows a Gamma distribution (Gaussian for ADT). Multiplet predictions as distribution mixtures estimated with goodness of fit for individual modalities. Weights based on goodness of fit, overall inference as weighted sum. Predictions from the ATAC modality lack sensitivity and downweighted. Comparable performance with other methods (scDblFinder, DoubletFinder, DoubletDetection, scds, Scrublet, and, for scATAC-seq, scDblFinder and AMULET). Python, [web app](https://shiny.crc.pitt.edu/shinyproj_composite/). <details>
  <summary>Paper</summary>
  Hu, Haoran, Xinjun Wang, Site Feng, et al. “A Unified Model-Based Framework for Doublet or Multiplet Detection in Single-Cell Multiomics Data.” Nature Communications 15, no. 1 (2024): 5562. https://doi.org/10.1038/s41467-024-49448-x.
</details>
...
- [MIRA](https://github.com/cistrome/MIRA) - scRNA-seq and scATAC-seq multiomics integration. Topic modeling (variational autoencoder) of cell states and regulatory potential modeling  (linear integration) of individual gene loci. Local chromatin accessibility-influenced transcriptional expression (LITE) genes, and nonlocal (NITE) genes. Biology of hair follicle differentiation, pseudotime trajectory analysis. Python. <details>
  <summary>Paper</summary>
  Lynch, Allen W., Christina V. Theodoris, Henry W. Long, Myles Brown, X. Shirley Liu, and Clifford A. Meyer. “MIRA: Joint Regulatory Modeling of Multimodal Expression and Chromatin Accessibility in Single Cells.” Nature Methods 19, no. 9 (2022): 1097–108. https://doi.org/10.1038/s41592-022-01595-z.
</details>
...
- [scETM](https://github.com/hui2000ji/scETM) - single-cell Embedded Topic Model for clustering and integration. A generative topic modeling that simultaneously learns the encoder network parameters and interpretable gene embeddings, topic embeddings, and batch effect linear intercepts from scRNA-seq data. Tri-factoring gene matrix into cell-by-topics, topics-by-embeddings, and embeddings by genes (Figure 1). Compared with scVI, scVI-LD, Seurat, scVAE-GM, Scanorama, Harmony, Liger on six published datasets. Harmony and Seurat performs OK, LIGER overcorrrects. Insensitive to doublets. Demo of pathway enrichment analysis on topics. Methods - softmax to impute dropouts, LDA description as it relates to transcriptomics, 2-layer neural network for parameter inference. Implemented in PyTorch, integrated with scanpy and tensorboard. Based on [ETM](https://github.com/adjidieng/ETM) - topic modeling in embedding spaces. <details>
  <summary>Paper</summary>
  Zhao, Yifan, Huiyu Cai, Zuobai Zhang, Jian Tang, and Yue Li. “Learning Interpretable Cellular and Gene Signature Embeddings from Single-Cell Transcriptomic Data.” Nature Communications 12, no. 1 (2021): 5261. https://doi.org/10.1038/s41467-021-25534-2.
</details>
...
- [harmony-pytorch](https://github.com/lilab-bcb/harmony-pytorch) - Pytorch implementation of Harmony algorithm on single-cell sequencing data integration.
...
- [mLLMCelltype](https://github.com/cafferychen777/mLLMCelltype) - An iterative multi-LLM consensus framework for cell type annotation in single-cell RNA sequencing data. By leveraging multiple large language models (GPT, Claude, Gemini, Qwen), it significantly improves annotation accuracy while providing transparent uncertainty quantification. Features include structured deliberation process, hallucination reduction through cross-model evaluation, robust performance with imperfect marker gene lists, and seamless integration with Scanpy/Seurat workflows. Available in both R and Python implementations. <details>
    <summary>Paper</summary>
...
- [Cellwhisperer](https://cellwhisperer.bocklab.org) - chat-based interrogation of human scRNA-seq data. Multimodal embedding of transcriptomes (Geneformer) and their textual annotation (preprocessed, BioBERT) (contrastive learning, CLIP method developed for multimodal embedding of images and text). Trained on GEO (bulk, processed by ARCHS4, and single-cell) and CELLxGENE census (over a million RNA-seq - text pairs). MetaSRA for text annotations. Mixtral 8x7B open-weights LLM for chat queries. Outperforms other sc Foundational Models (scGPT, UCE for transcriptome embedding, BioGPT for text embedding) in predicting cell characteristics, marker gene identification. Benchmarked on human development scRNA-seq, Tabula Sapiens, ImmGen, Pancreas, Colonic scRNA-seq, more. Integrated with [CELLxGENE](https://cellxgene.cziscience.com/docs/01__CellxGene) browser, [GEO datasets](https://cellwhisperer.bocklab.org/geo/). API to 1) chat queries to retrieve information about cells, and 2) get Cellwhisperer scores for queries. [GitHub](https://github.com/epigen/cellwhisperer). <details>
  <summary>Paper</summary>
  Schaefer, Moritz, Peter Peneder, Daniel Malzl, et al. “Multimodal Learning Enables Chat-Based Exploration of Single-Cell Data.” Nature Biotechnology, ahead of print, November 11, 2025. https://doi.org/10.1038/s41587-025-02857-9.
</details>
...
- [DeepSpot2Cell](https://github.com/ratschlab/DeepSpot2Cell) - Python package for virtual single-cell spatial transcriptomics prediction from H&E images using spot-level supervision. NeurIPS 2025 Imageomics. <details>
  <summary>Paper</summary>
...
- [FlashDeconv](https://github.com/cafferychen777/flashdeconv) - a high-performance spatial transcriptomics deconvolution method using structure-preserving randomized sketching for atlas-scale and subcellular-resolution platforms (Visium HD, Stereo-seq, Xenium). Employs leverage-score importance sampling to preserve transcriptomically distinct markers, preventing rare cell type signal loss. Processes 1 million spots in ~3 minutes with linear O(N) scaling for both time and memory. No GPU required. Benchmarking on Spotless suite shows Pearson correlation of 0.944, matching top-tier Bayesian methods while accelerating inference by orders of magnitude. Python implementation, scanpy-compatible API. [PyPI](https://pypi.org/project/flashdeconv/). <details>
  <summary>Paper</summary>
  Yang, Chen, Jun Chen, and Xianyang Zhang. "FlashDeconv enables atlas-scale, multi-resolution spatial deconvolution via structure-preserving sketching." bioRxiv (2025). https://doi.org/10.64898/2025.12.22.696108.
</details>
...
- [STDH](https://github.com/yi-zhang/STHD) (probabilistic cell typing of single Spots in whole Transcriptome spatial data with High Definition) - a machine learning method for probabilistic cell typing of single spots in whole-transcriptome high-resolution spatial transcriptomics data (10x VisiumHD). Directly models gene expression at single-spot level to infer cell type identities without cell segmentation or spot aggregation. Addresses sparsity by modeling count statistics, incorporating neighbor similarities, and leveraging reference single-cell RNA-seq data. Two-component loss function - optimizes log likelihood of spot gene counts (Poisson distribution) with the latent cell type variable (reference annotated scRNA-seq dataset required), and denoises sparsity using cross entropy loss based on neighborhood similarity in cell type probabilities. Outputs cell type probabilities and labels for each high-resolution spot. Outperforms six six cell typing methods (the deconvolution-based cell typing tools [RCTD](https://github.com/dmcable/spacexr), [CARD](https://yma-lab.github.io/CARD/documentation/04_CARD_Example.html), [Cell2Location](https://cell2location.readthedocs.io/en/latest/notebooks/cell2location_tutorial.html), [Celloscope](https://github.com/szczurek-lab/Celloscope), [Stereoscope](https://github.com/almaan/stereoscope), and the unsupervised method [Banksy](https://github.com/prabhakarlab/Banksy_py/blob/main/slideseqv2_analysis.ipynb)) on simulated and public experimental data. Interactive [STHDviewer](https://yi-zhang-compbio-lab.github.io/STHDviewer_colon_cancer_hd/STHDviewer_crchd.html). <details>
    <summary>Paper</summary>
    Sun, Chuhanwen, and Yi Zhang. "STHD: probabilistic cell typing of single Spots in whole Transcriptome spatial data with High Definition." bioRxiv (2024): 2024-06. https://doi.org/10.1101/2024.06.20.599803
</details>
...
- [BANKSY](https://github.com/prabhakarlab/Banksy) (Building Aggregates with a Neighborhood Kernel
and Spatial Yardstick) - highly scalable spatial transcriptomics analysis tool to define both cell types and tissue domains. Embeds cells in a product space of their own and the local neighbourhood transcriptome (cell state and microenvironment). Uses a pair of spatial kernels to encode the transcriptomic structure of the microenvironment, one using the weighted mean of gene expression in each cell's neighborhood and the other using an azimuthal Gabor filter (AGF, capturing to the gradient of gene expression in each cell's neighborhood). A single hyperparameter (mixing parameter) can be used to switch between calling cell types or tissue domains. Can be used to find spatial niche-specific cell states, denoise clustering results, run in multisample mode to do spatially informed batch correction, and for domain finding. Applied to various technologies (Slide-seq v1, v2, MERSCOPE, CosMx, VeraFISH, 10x Visium, spatial proteomics), it detects undescribed biological features. Outperforms the MERINGUE, SpaGCN, Giotto, BayesSpace, SpiceMix, FICT, GraphST, STAGATE methods. Fast, scalable, robust to parameter estimation. [R](https://github.com/prabhakarlab/Banksy) and [Python](https://github.com/prabhakarlab/Banksy_py) implementations, [Seurat wrapper](https://github.com/satijalab/seurat-wrappers/blob/master/docs/banksy.md). [Scripts](https://zenodo.org/records/10258795) to reproduce results. <details>
    <summary>Paper</summary>
...
- [Celloscope](https://github.com/szczurek-lab/Celloscope) - marker gene-based cell type deconvolution in spatial transcriptomics data. A Bayesian probabilistic graphical model. Binary matrix of marker gene x cell type as prior knowledge. Cell type counts in each spot as prior for the hidden variable for the total number of cells. Considers unknown cell type. Outperforms Stereoscope, RCTD, SpatialDWLS, CellAssign and immune deconvolution methods ABIS, EPIC, quanTIseq, xCell on simulated data, spatially distinguishes brain structures, dissects immune microenvironment in prostate gland tissue. Robust to noise, marker gene selection, fast. Python. <details>
  <summary>Paper</summary>
  Agnieszka Geras, Shadi Darvish Shafighi, Kacper Domżał, Igor Filipiuk, Alicja Rączkowska, Paulina Szymczak, Hosein Toosi, Leszek Kaczmarek, Łukasz Koperski, Jens Lagergren, Dominika Nowis and Ewa Szczurek. Celloscope: a probabilistic model for marker-gene-driven cell type deconvolution in spatial transcriptomics data. Genome Biol 24, 120 (2023). https://doi.org/10.1186/s13059-023-02951-8
</details>
...
- **snmCAT-seq** - single-nucleus methylcytosine, chromatin accessibility, and transcriptome sequencing.
Table 1 - other multi-omics technologies. [Protocol](https://www.protocols.io/view/snmcat-v1-n92ld914xg5b/v1). Applied to human brain cortical cell types, 4,358 cells. Diverse correlation between mCH and gene expression (3 types), best for cell type specific expression. Pathway analysis using [SynGO](https://www.syngoportal.org/). Correlation of TF binding with hyper and hypomethylated regions is also diverse. Identification of open chromatin regions from methylation using [gNOMePeaks](https://github.com/karl616/gNOMePeaks). snATAC-seq data processing with [SnapATAC](https://github.com/r3fang/SnapATAC). Excellent Methods for multi-modal data integration/fusion, concatenation of informative PCs from individual modality PCA, then UMAP. Non-CG methylation is the most distinguishing cell types, followed by CG methylation, RNA abundance. Detailed strategies for clustering and manifold learning (consensus clustering on concatenated PCs, evaluating clustering reproducibility and removing outliers, resolution choice). Integration with snRNA-seq using a mutual nearest neighbor approach. Clustering with cross-validation, clustering DNA methylation at different resolution, testing how well it is supported by transcriptomics, 20-50 clusters most optimal. Co-embedding details, intuition, Cross-modality k-partner graph in  the shared low dimensional space, distance calculations. Over- and undersplitting scores, [analysis](https://github.com/FangmingXie/mctseq_over_under_splitting/blob/master/over-under-splitting-analysis.ipynb). New data fusion method [SingleCellFusion](https://github.com/mukamel-lab/SingleCellFusion), for multi-omics integrationg (DNA methylation, ATAC-seq, RNA-seq data.) Four steps: preprocessing, within-modality smoothing, cross-modality imputation, clustering and visualization. Outperforms Scanorama, Harmony, LIGER, comparable with Seurat. <details>
  <summary>Paper</summary>
  Luo, Chongyuan, Hanqing Liu, Fangming Xie, et al. “Single Nucleus Multi-Omics Identifies Human Cortical Cell Regulatory Genome Diversity.” Cell Genomics 2, no. 3 (2022): 100107. https://doi.org/10.1016/j.xgen.2022.100107.
</details>
...
- [scBaseCount](https://github.com/ArcInstitute/arc-virtual-cell-atlas) - AI-curated uniformly processed 10x scRNA-seq data repository. Over 230 million cells spanning 21 organisms and 72 tissues. [SRAgent](https://github.com/ArcInstitute/SRAgent) agentic workflow built with [LangGraph](https://github.com/langchain-ai/langgraph) to scan eSearch, eSummary, eFetch, eLink, NCBI HTML scraping, SRA BigQuery, sra-stat, and fastq-dump. [scRecounter](https://github.com/ArcInstitute/scRecounter) Nextflow pipeline, 10x references, STARsolo. All STARsolo count matrices are located on Google Cloud Storage at gs://arc-ctc-scBaseCount/2025-02-25. [Documentation](https://github.com/ArcInstitute/arc-virtual-cell-atlas/blob/main/scBaseCount/README.md). <details>
  <summary>Paper</summary>
  scBaseCount: an AI agent-curated, uniformly processed, and continually expanding single cell data repository. Nicholas D. Youngblut, Christopher Carpenter, Jaanak Prashar, Chiara Ricci-Tam, Rajesh Ilango, Noam Teyssier, Silvana Konermann, Patrick D. Hsu, Alexander Dobin, David P. Burke, Hani Goodarzi, Yusuf H. Roohani. bioRxiv 2025.02.27.640494; doi: https://doi.org/10.1101/2025.02.27.640494
</details>
