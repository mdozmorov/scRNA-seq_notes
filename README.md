...
and Spatial Yardstick) - spatial transcriptomics analysis to define both cell types and tissue domains. Embeds cells in a product space of their own and the local transcriptome (cell state and microenvironment). Uses a pair of spatial kernels to encode the transcriptomic structure of the microenvironment, one using the weighted mean of gene expression in each cell's neighborhood and the other using an asimuthal Gabor filter (AGF, analogous to the gradient of gene expression in each cell's neighborhood). A single hyperparameter (mixing parameter) to focus either on cell types or tissue domains. Can be used to denoise clustering results, act as an approximate deconvolution method, run in multisample mode to do spatially informed batch correction. Applied to various technologies (Slide-seq v1, v2, MERSCOPE, CosMx, VeraFISH, 10x Visium, spatial proteomics), detects undescribed biological features. Outperforms MERINGUE< SpaGNN, Giotto, BayesSpace, SpiceMix, FICT, GraphST, STAGATE methods. Fast, scalable, robust to parameter estimation. [R](https://github.com/prabhakarlab/Banksy) and [Python](https://github.com/prabhakarlab/Banksy_py) implementations, [Seurat wrapper](https://github.com/satijalab/seurat-wrappers/blob/master/docs/banksy.md). [Scripts](https://zenodo.org/records/10258795) to reproduce results. <details>
    <summary>Paper</summary>
